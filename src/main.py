...
async def chat_command(int: discord.Interaction, message: str):
    try:
        ...
        try:
            ...
            logger.exception(e)
            await int.response.send_message(f"Failed to start chat {str(e)}",
                                            ephemeral=True)
            return
        ...
        async with thread.typing():
            ...
            # send the result
            await process_response(user=user,
                                   thread=thread,
    ...
    try:
        ...
        if len(blocked_str) > 0:
            try:
                ...
                await thread.send(embed=discord.Embed(
                    description=
                    f"❌ **{message.author}'s message has been blocked by moderation but could not be deleted. Missing Manage Messages permission in this Channel.**",
                    color=discord.Color.red(),
                ))
                return
        await send_moderation_flagged_message(
        ...
        # send response
        await process_response(user=message.author,
                               thread=thread,
                               response_data=response_data)
    except Exception as e:
        logger.exception(e)
